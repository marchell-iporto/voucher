...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            setTimeout(function() {
                console.log('🔄 Running delayed account mapping fix...');
                fixAccountMapping();
            }, 500);
        ...
        // Function untuk memperbaiki mapping account
        function fixAccountMapping() {
            console.log('🔧 Fixing account mappings...');
            
            $('.account-row').each(function(index) {
                const $row = $(this);
                const $select = $row.find('.account-name-select');
                const accountNumber = $row.find('.account-number-display').val();
                
                if (accountNumber && !$select.val()) {
                    const matchingAccount = accounts.find(acc => acc.nomor_akun === accountNumber);
                    
                    if (matchingAccount) {
                        console.log(`🎯 Row ${index + 1}: Mapping ${accountNumber} to ${matchingAccount.nama_akun}`);
                        
                        if ($select.hasClass('select2-hidden-accessible')) {
                            $select.select2('destroy');
                        }
                        
                        $select.val(matchingAccount.nama_akun);
                        $row.find('.account-number-hidden').val(accountNumber);
                        
                        initializeSelect2ForElement($select);
                        console.log(`✅ Fixed row ${index + 1}`);
                    }
                }
            });
        }
        ...
        function initializeValidation() {
            $('#voucherForm').validate({
                ...
                messages: {
                    ...
                    from_to: {
                        required: "Receive From wajib diisi",
        ...
        window.debugAccountData = function() {
            console.log('=== RECEIVE VOUCHER DEBUG ===');
        ...
        window.fixAccountMapping = function() {
            fixAccountMapping();
        };
    </script>
</body>
